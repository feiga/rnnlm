...
flags.DEFINE_integer("dim", 1500, "Hidden dimension of each lstm layer")
...
class PTBModel(object):
  ...
  def __init__(self, is_training, config, input_):
    ...
    if FLAGS.resnet:
      def res_cell():
        return tf.contrib.rnn.ResidualWrapper(attn_cell())
    cell = tf.contrib.rnn.MultiRNNCell(
        [res_cell() for _ in range(config.num_layers)], state_is_tuple=True)
...
class LargeConfig(object):
  """Large config."""
  init_scale = 0.1#0.08 #0.04
  learning_rate = 20#1.0
  max_grad_norm = 0.25#5 #10
  num_layers = 2
  num_steps = 35
  hidden_size = 1500
  max_epoch = 14
  max_max_epoch = 10000#39 #55
  keep_prob = 0.2 #0.35
  lr_decay = 1 / 4#1.5
  batch_size = 20
  vocab_size = 10000
...
def main(_):
  ...
  eval_config.keep_prob = FLAGS.dropout
  ...
  with tf.Graph().as_default():
    ...
    last_valid = 1000000000
    curr_valid = 1000000000
    best_valid = 1000000000
    curr_lr = config.learning_rate
    with sv.managed_session() as session:
      for i in range(config.max_max_epoch):
        #lr_decay = config.lr_decay ** max(i + 1 - config.max_epoch, 0.0)
        if curr_valid > last_valid:
          curr_lr *= config.lr_decay
        ...
        if valid_perplexity < best_valid:
          ...
          print("Best Batched Valid Perplexity improved to %.03f" % best_valid)
          if FLAGS.save_path:
